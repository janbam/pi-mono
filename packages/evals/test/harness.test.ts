...
describe("documentation variant", () => {
	...
	// JBMOD: the fork's default prompt has no Pi documentation section, so the docs-stripping eval variant does not apply.
	it.skip("strips only the documentation routing section from the default Pi prompt", () => {
	...
	// JBMOD: depends on the Pi documentation section the fork omits (see above).
	it.skip("verifies the prompt that was sent", () => {
		const prompt = buildSystemPrompt({
			cwd: "/workspace",
			selectedTools: [...DOCUMENTATION_EVAL_TOOLS],
		});
