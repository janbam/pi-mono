...
export function streamSimple<TApi extends Api>(
	...
	if (builtinProvider) {
		...
		return builtinProvider.streamSimple(model, transcript, withEnvApiKey(model, resolvedOptions));
	}
	const provider = resolveApiProvider(model.api);
	const resolvedOptions = resolveModelSimpleStreamOptions(model, options);
	return provider.streamSimple(model, transcript, withEnvApiKey(model, resolvedOptions));
}
