...
function capturePromptCacheWarmupInvariants(
	params: MessageCreateParamsStreamingWithFallbacks,
): PromptCacheWarmupInvariants {
	return {
...
// Warmups dispatch non-streaming zero-token requests, so the streaming flag is decided
// at request time rather than through the SDK's literal-typed streaming/non-streaming union.
type MessageCreateParamsStreamingWithFallbacks = Omit<MessageCreateParamsStreaming, "stream"> & {
	stream: boolean;
