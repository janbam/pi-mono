...
Cycle through available thinking levels. Returns `null` data if the model doesn't support thinking. Optional `direction` is `"forward"` (default) or `"backward"`.
...
These fork-owned commands let a controlled RPC host discover and execute Pi tools without starting an agent turn. Neither the command nor its result is appended to conversation context. Tool argument preparation, schema validation, extension hooks, and tool lifecycle events still run. UI requests during `execute_tool` use print-mode semantics: dialogs return cancelled or default values, and notifications are dropped.

### get_all_tools

List all registered tools, including tools inactive for model use. Each item includes its name, description, input `parameters`, optional `promptGuidelines`, and `sourceInfo`.
...
```json
{"type": "response", "command": "get_all_tools", "success": true, "data": {"tools": [{"name": "read", "description": "Read a file", "parameters": {"type": "object"}, "sourceInfo": {"source": "builtin", "scope": "package"}}]}}
```

### execute_tool

Execute a registered tool by name. `toolCallId` is optional and is generated when absent. Use it to correlate `tool_execution_start`, `tool_execution_update`, and `tool_execution_end` events with the final response.

```json
{"type": "execute_tool", "toolName": "read", "input": {"path": "README.md"}, "toolCallId": "host-read-1"}
```

```json
{"type": "response", "command": "execute_tool", "success": true, "data": {"toolName": "read", "toolCallId": "host-read-1", "content": [{"type": "text", "text": "# Project"}], "details": {}, "isError": false}}
```

Unknown names and invalid input return `success: false` because execution never started. Blocked calls, aborts, and execution failures return `success: true` with `data.isError: true`.
