...
Extensions can also store session-global JSON values outside that tree and model context. `/tree` does not roll them back. New sessions start empty; `/fork`, `/clone`, CLI `--fork`, and branch JSONL exports inherit one effective value per key.
...
In `/tree`, press `Enter` on a user message to put its text back in the editor without a summary. Edit and submit it to create another branch. Press `Enter` on another entry to continue after it with an empty editor. Press `Tab` to navigate and open the branch-summary menu.
...
Use `/export` to write the current session as HTML, JSONL, or Markdown. Markdown export omits thinking blocks and renders tools as in the collapsed transcript. Use `/share` to upload the session and get a viewer link. Pi uses a Radius artifact when Radius authentication is configured; otherwise, it uses a private GitHub gist.
