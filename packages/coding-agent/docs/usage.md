...
Pi shows each tool call and result while it works. Press `Ctrl+O` to expand or collapse tool output. Press `Ctrl+H` to show or hide thinking blocks.

The startup header lists the instructions and resources Pi loaded. The editor border indicates the current thinking level. The footer updates as the model uses context and reports usage. After each foreground run, Pi prints uncached input, output, cache reads, cache writes, and total cost, including usage reported by tools. Compaction and branch-summary requests are excluded.
...
| Pause before the next model request | Press `Escape`; current tool calls finish first |
| Abort the active stream and tools | Press `Ctrl+Escape` or `Ctrl+\\` |

A message sent with `Enter` waits until the current response and its tool calls finish, then guides the next response. A follow-up sent with `Alt+Enter` waits until Pi finishes the current task. While paused, type a message to continue with it or press `Escape` to continue without one. Pressing `Escape` again before the pause lands cancels it. Aborting returns queued messages to the editor.
...
- `/thinking` selects how much reasoning the current model uses. Press `Ctrl+T` or `Ctrl+Alt+T` to cycle forward or backward through supported levels.
- `/login` and `/logout` manage provider access.
- `/settings` changes common preferences.
- `/warm [on|off]` reports or overrides prompt cache warming for this process. `-kw` or `--keep-cache-warm` enables it at startup. See [Cache Warming](settings.md#cache-warming).
...
Press `Ctrl+X` or run `/copy` to copy the last assistant response. Use `/export` to save the session as HTML, JSONL, or Markdown. A `.md` filename writes the visible conversation without thinking blocks, with tools rendered as in the collapsed transcript.
