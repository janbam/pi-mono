...
| `fullscreenWheelScrollLines` | number | `1` | Logical lines moved per normal mouse-wheel event in fullscreen mode; Alt-wheel moves one line for precision. Values are rounded down and clamped to at least 1. Has no effect in regular TUI mode |
...
| `branchSummary.skipPrompt` | boolean | `false` | Obsolete: `enter` in `/tree` always navigates without a summary prompt; use `tab` to open the summary menu. This setting no longer has an effect. |
