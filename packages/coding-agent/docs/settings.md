...
| `cacheWarming` | `"off" \| "streaming" \| "idle"` | `"off"` | Keep eligible provider prompt caches warm during active runs or, with `"idle"`, between runs. Global setting only. |
| `cacheWarmingMaxAgeMinutes` | number | `60` | Maximum age since the last real request for warming while running or idle. Global setting only. |

<a id="cache-warming"></a>

Cache warming requires a declared cache lifetime. The `"streaming"` and `"idle"` modes require at least $0.05 in expected savings; idle mode uses a 15% continuation estimate. `--keep-cache-warm` (`-kw`) and `/warm on` enable process-only warming while running and idle without that savings floor. `/warm off` disables it for the process, and bare `/warm` reports its state. These commands do not change the saved setting.

Pi replays the last request ten seconds before expiry. Anthropic Messages models, including eligible proxies, use `max_tokens: 0`; other APIs use a one-token cap. A refresh without a cache read or a failed refresh stops warming. Resuming a session can restore a still-valid lease from the last request. Warming also stops on context changes or when `cacheWarmingMaxAgeMinutes` elapses since the last real request.

Refresh usage counts toward session totals but does not enter model context. `/session` shows the next decision, and an indicator above the editor shows held-warm time, refresh count, and cost while warming is enabled. Extensions can override a decision with `cache_warming_decision`. See [Prompt Cache Lifetimes](models.md#prompt-cache-lifetimes).
...
| `branchSummary.skipPrompt` | boolean | `false` | Obsolete in this fork: `Enter` in `/tree` navigates directly; `Tab` opens the summary menu. |
...
| `fullscreenWheelScrollLines` | number | `1` | Logical lines per normal fullscreen wheel event, rounded down and clamped to at least 1. Alt-wheel moves one line. |
