...
For one-off requests the registry offers two entry points. `ctx.modelRegistry.complete(model, context, options)` takes api-specific request options, such as `reasoningEffort` for OpenAI-style apis, and sends them as given. `ctx.modelRegistry.completeSimple(model, context, options)` takes provider-neutral options instead, including `reasoning` as a thinking level (`minimal`, `low`, `medium`, `high`, `xhigh`, `max`); each adapter encodes that level in its own request shape. Omit `reasoning` to ask for no thinking — adapters treat a present level as thinking-on, so there is no `off` value here. Clamp a user-supplied level with `clampThinkingLevel(model, level)` from pi-ai before passing it, and send no `reasoning` when the clamp returns `off`; only some adapters clamp on their own, and a model that cannot disable thinking must be asked for its lowest level instead of a disabled one. If all you have is a thinking level, use `completeSimple` rather than translating levels into api options yourself.
