...
For one-off requests the registry offers two entry points. `ctx.modelRegistry.complete(model, context, options)` takes api-specific request options, such as `reasoningEffort` for OpenAI-style apis, and sends them as given. `ctx.modelRegistry.completeSimple(model, context, options)` takes provider-neutral options instead, including `reasoning` as a pi thinking level (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). The runtime clamps the requested level to what the model supports, then each adapter encodes the effective level in its own request shape. Omitted `reasoning` means `off`; when a model cannot disable thinking, both omission and explicit `off` use its nearest supported level. Use `completeSimple` when the caller knows the intended thinking level rather than provider-specific request fields.
