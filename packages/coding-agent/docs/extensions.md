...
| Persist branch-local non-context data | `pi.appendEntry()` |
| Persist session-global JSON outside the tree | `pi.getSessionState()` and `pi.setSessionState()` |
| Run a registered tool directly | `pi.executeTool()` |
...
For one-off model requests, `ctx.modelRegistry.completeSimple(model, context, options)` accepts the provider-neutral Pi thinking level in `options.reasoning`. The runtime clamps it to the model's supported levels before the adapter encodes it. Omission means `off`, or the nearest supported level when the model cannot disable thinking. Use `complete()` when the caller deliberately supplies API-specific request options.
...
`pi.executeTool(name, input, options?)` runs any registered tool by name, including one inactive for model use. Inspect names and schemas with `pi.getAllTools()` first. It prepares and validates arguments, runs tool hooks and lifecycle events, and returns the result without appending a `toolResult` message or starting an agent turn. Unknown names and invalid input throw; blocked calls, aborts, and execution failures resolve with `isError: true`.
...
| Branch-local durable data excluded from model context | `pi.appendEntry()` |
| Session-global JSON excluded from tree and model context | `pi.getSessionState()` and `pi.setSessionState()` |
...
Session-global keys share one open last-write-wins namespace. Values may be JSON objects, arrays, strings, finite numbers, booleans, or `null`; pass `undefined` to clear. A failed setter leaves the last durable value unchanged. State survives resume and reload, while new sessions start empty. Derived sessions inherit one effective value per key before outgoing `session_shutdown`; writes from that shutdown stay source-only. See [Session File Format](session-format.md#sessionstateentry) and the [session-state example](../examples/extensions/session-state.ts).

`pi.registerShortcut()` defaults to the editor context. Add `contexts: ["editor", "tree"]` to dispatch in `/tree` too. A tree invocation includes the selected entry while the original branch is still active. The handler may return `navigateTo`, `editorText`, or `reopenTree`; tree selector bindings take priority, and editor-reserved keys remain unavailable in both contexts. See the [tree-shortcut example](../examples/extensions/tree-shortcut.ts).
