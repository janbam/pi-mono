...
This fork also stores session-global extension JSON outside that tree. `getSessionState(key)` reads the effective value, `setSessionState(key, value)` appends a write or clears the key with `undefined`, and `getSessionStateSnapshot()` returns a defensive snapshot. `getEntries()` still returns only conversation entries. New sessions start empty; `/tree` does not roll state back. Forks, clones, and branch exports copy one effective value per key before outgoing `session_shutdown`, so shutdown writes remain in the source session. State writes flush immediately; validation or persistence failure leaves the last durable value unchanged.
...
`AgentSession.executeTool(toolName, input, options?)` runs a registered tool directly without starting an agent turn or appending a tool-result message. Discover its schema with `session.getAllTools()`. Direct execution prepares and validates arguments, runs `tool_call` and `tool_result` hooks, and emits tool lifecycle events. Unknown tools and invalid input throw; blocked calls, aborts, and execution failures resolve with `isError: true`.

`cycleThinkingLevel(direction?)` accepts `"forward"` or `"backward"`; omission cycles forward.
