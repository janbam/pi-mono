...
function createTreeUI() {
	...
	const ui = {
		...
		// Fork: tree confirmation delegates the committed navigation to this shared helper.
		performTreeNavigation: Reflect.get(InteractiveMode.prototype, "performTreeNavigation"),
	};
	...
	return {
		...
		async select() {
			...
			// Fork: the summary dialog these cases exercise opens only on the summary-menu confirm (tab).
			await selector!.getTreeList().onSelect!(targetId, true);
		},
	};
}
