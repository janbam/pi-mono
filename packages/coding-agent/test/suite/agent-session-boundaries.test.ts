...
describe("AgentSession actionable boundaries", () => {
	...
	it("keeps a boundary replacement verbatim through threshold compaction", async () => {
		let handled = false;
		const requests: string[] = [];
		// JBMOD: sized so the replacement alone crosses the threshold with the fork's shorter system prompt.
		const instruction = "EXACT-REPLACEMENT-INSTRUCTION ".repeat(140);
		const harness = await createHarness({
