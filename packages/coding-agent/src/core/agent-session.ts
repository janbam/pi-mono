...
export interface AgentSessionConfig {
	...
	cwd: string;
	/** Exclude cwd from the system prompt, not from tool or session paths. */
	noCwd?: boolean;
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this._noCwd = config.noCwd ?? false;
		...
		// Installed last so a landed pause can override the boundary hooks' continuation decision.
	...
	async executeTool(
		...
		let toolCall: AgentToolCall = {
			...
			id: toolCallId,
			name: toolName,
			// Direct callers pass decoded JSON input; schema validation below enforces the tool's argument shape.
	...
	private _installPauseHook(): void {
		const previousFinishTurn = this.agent.finishTurn;
		this.agent.finishTurn = async (turn, signal): Promise<AgentTurnDecision | undefined> => {
			...
			// Consume the request and hold the run before the next LLM call; queues stay untouched for resume.
	...
	/**
	 * Continue a held run without adding a new message.
	 *
	 * Mirrors _runAgentPrompt's lifecycle so post-run retry/compaction handling,
	 * boundary dispatch, bash flushing, and idle signaling stay consistent.
	 */
	private async _runAgentContinuation(): Promise<void> {
		this._agentRunAbortRequested = false;
		this._isAgentRunActive = true;
		try {
			// Settle what the pause froze first (staged retry or compaction); either way exactly one continuation follows.
	...
	/** Drive post-run recovery and pre-settle boundaries until the run settles, aborts, or a pause holds it. */
	private async _settleAgentRun(): Promise<void> {
		while (!this._agentRunAbortRequested) {
			...
			// A held pause is not settled: agent_before_settle fires once the resumed run really ends.
			if (this._pausedResumable) break;
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		if (this._pausedResumable) return false;
