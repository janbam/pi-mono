...
/**
 * Scroll position label: percentage of scrollable distance, or "All" when the
 * content fits the viewport. Pure function so tests can pin the arithmetic.
 */
export function formatPositionLabel(scrollTop: number, contentHeight: number, viewportHeight: number): string {
	const maxScrollTop = Math.max(0, contentHeight - viewportHeight);
	if (maxScrollTop === 0) return "All";
	return `${Math.min(100, Math.round((scrollTop / maxScrollTop) * 100))}%`;
}
